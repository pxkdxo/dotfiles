...
if mkdir -p -- "${BASH_CACHE_DIR}"; then
...
add_precmd_functions() {
  while (($#)); do
    if (IFS=':' && [[ ":${precmd_functions[*]}:" != *:"$1":* ]]); then
      precmd_functions+=("$1")
    fi
    shift
  done
...
add_preexec_functions() {
  while (($#)); do
    if (IFS=':' && [[ ":${preexec_functions[*]}:" != *:"$1":* ]]); then
      preexec_functions+=("$1")
    fi
    shift
  done
...
remove_precmd_functions() {
  while (($#)); do
    for _ in "${!precmd_functions[@]}"; do
      if [[ $1 == "${precmd_functions[_]}" ]]; then
...
remove_preexec_functions() {
  while (($#)); do
    for _ in "${!preexec_functions[@]}"; do
      if [[ $1 == "${preexec_functions[_]}" ]]; then
