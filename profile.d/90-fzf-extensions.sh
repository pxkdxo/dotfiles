#!/usr/bin/env sh
# ~/.profile.d/90-fzf-extensions.sh: fzf extension functions
...
if command -v fzf > /dev/null 2>&1; then
  ...
  CTRL-D : Enter Directory   CTRL-/: Toggle Preview   ESC : Exit")"
    ...
    _selected=$(_fzf_find_files "${_search_path}" | fzf \
      --multi \
    ...
    unset _search_path _editor_cmd _open_cmd _clipboard_cmd _preview_cmd _dir_preview_cmd _header _selected
  }
  ...
  CTRL-O : Copy Path   CTRL-/: Toggle Preview   ESC : Exit")"
    ...
    _selected=$(_fzf_find_dirs "${_search_path}" | fzf \
    ...
    unset _search_path _editor_cmd _clipboard_cmd _preview_cmd _header _selected
  }
  ...
  # Select from git status (modified, staged, untracked) - stage, unstage, diff, open
  fzf_git_status() {
    if ! command -v git > /dev/null; then
      echo "git is not installed" >&2
      return 1
    ...
    # Preview: show diff (unstaged and staged) for selected file
    # shellcheck disable=SC2016
    _preview_cmd='_p=$(echo "{}" | cut -c4-); case "$_p" in *" -> "*) _p="${_p#* -> }"; esac; git diff -- "$_p" 2>/dev/null; echo "--- STAGED ---"; git diff --cached -- "$_p" 2>/dev/null'

    _header="$(_fzf_header "
  RETURN : Stage   CTRL-U : Unstage   CTRL-X : Open   ALT-X : Edit
  CTRL-D : Diff in Editor   CTRL-O : Copy Path   CTRL-/: Toggle Preview   ESC : Exit")"

    # Path extraction: cut -c4- skips "XY " (status + space); for renames use path after " -> "
    # shellcheck disable=SC2016
    _stage_cmd='printf "%s\n" {+} | while IFS= read -r _l; do _p=$(echo "$_l" | cut -c4-); case "$_p" in *" -> "*) _p="${_p#* -> }"; esac; git add "$_p"; done'
    _unstage_cmd='printf "%s\n" {+} | while IFS= read -r _l; do _p=$(echo "$_l" | cut -c4-); case "$_p" in *" -> "*) _p="${_p#* -> }"; esac; git reset HEAD -- "$_p"; done'
    _path_cmd='_p=$(echo "{}" | cut -c4-); case "$_p" in *" -> "*) _p="${_p#* -> }"; esac'

    _selected=$(printf '%s\n' "${_status}" | fzf \
      --multi \
      --ansi \
      --bind="enter:execute-silent(${_stage_cmd})+reload(git status -s)" \
      --bind="ctrl-u:execute-silent(${_unstage_cmd})+reload(git status -s)" \
      --bind="ctrl-x:execute-silent(${_path_cmd}; ${_open_cmd} \"\$_p\")+abort" \
      --bind="alt-x:execute(${_path_cmd}; ${_editor_cmd} \"\$_p\")+abort" \
      --bind="ctrl-d:execute(${_path_cmd}; (git diff -- \"\$_p\"; echo '--- STAGED ---'; git diff --cached -- \"\$_p\") | ${_editor_cmd} -)+abort" \
      --bind="ctrl-o:execute-silent(${_path_cmd}; printf '%s\n' \"\$_p\" | ${_clipboard_cmd})" \
      --bind='ctrl-/:toggle-preview' \
      --preview="${_preview_cmd}" \
      --preview-window='right:60%' \
      --header="${_header}")

    unset _editor_cmd _open_cmd _clipboard_cmd _preview_cmd _header _stage_cmd _unstage_cmd _path_cmd _status _selected
  }
  ...
  fzf_cp() {
    ...
    _selected=$(_fzf_find_files_only "${_search_path}" | fzf --multi)

    if [ -z "${_selected}" ]; then
      unset _search_path _dst _selected
      return 0
    fi
  ...
  fzf_rm() {
    ...
    if command -v fd > /dev/null; then
      _reload_cmd="fd --hidden --follow --type f -- \"${_search_path}\""
    else
      _reload_cmd="find \"${_search_path}\" -type f"
    fi

    _selected=$(_fzf_find_files_only "${_search_path}" | fzf --multi \
      --bind="ctrl-x:execute-silent(rm {+})+reload(${_reload_cmd})" \
    ...
    unset _search_path _reload_cmd _selected
  }
  ...
  # Download media with yt-dlp (URLs passed as args)
  # Uses FZF_DLP_BROWSER for preview (default: open/xdg-open) and
  # FZF_DLP_TERMINAL for download terminal (default: alacritty, kitty, or xterm)
  fzf_dlp() {
    if test "$#" -eq 0; then
    ...
    _dlp_tmp=$(mktemp) || return 1
    printf '%s\n' "$@" > "${_dlp_tmp}"

    _open_cmd="${FZF_DLP_BROWSER:-$(_fzf_get_open)}"
    if command -v alacritty > /dev/null; then
      _term_cmd="${FZF_DLP_TERMINAL:-alacritty}"
    elif command -v kitty > /dev/null; then
      _term_cmd="${FZF_DLP_TERMINAL:-kitty}"
    else
      _term_cmd="${FZF_DLP_TERMINAL:-xterm}"
    fi

    fzf --multi \
      --bind="start,change:reload(cat \"${_dlp_tmp}\")" \
      --bind="ctrl-m:execute-silent(printf '%s\n' {+} | while IFS= read -r _u; do ${_open_cmd} \"\${_u}\" & done)" \
      --bind="ctrl-x:execute-silent(${_term_cmd} -e sh -c 'yt-dlp --quiet --progress -f \"bv*+ba/b\" -N 128 -- {+}; exec sh' &)"

    rm -f "${_dlp_tmp}"
    unset _dlp_tmp _open_cmd _term_cmd
  }
  ...
  alias fzgs='fzf_git_status'
  alias fzgc='fzf_git_commit'
  alias fzgf='fzf_git_files'
  alias fzgst='fzf_git_stash'
  ...
  fzgs, fzf_git_status      - Stage/unstage/diff files from git status
  fzgc, fzf_git_commit      - Browse git commits
  fzgf, fzf_git_files       - Select git-tracked files
  fzgst, fzf_git_stash      - Browse git stashes
