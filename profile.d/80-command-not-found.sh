...
if test -n "${ZSH_NAME}"; then
  ...
  __command_not_found_pkgfile_init="/usr/share/doc/pkgfile/command-not-found.zsh"
  __sh_ext="zsh"
...
if test -n "${__command_not_found_func:-}"; then
  # Flag for success
  __cnf_handler_set=0

  # Platforms with a built-in command-not-found handler init file
  for file in "${__command_not_found_pkgfile_init}" \
...
if test "${__cnf_handler_set}" -eq 0 && test -n "${__command_not_found_func:-}"; then
  # Platforms with manual command_not_found_handler() setup (bash/zsh only)
...
unset __cnf_handler_set
fi
unset __command_not_found_pkgfile_init
unset __command_not_found_func
