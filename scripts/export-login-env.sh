...
#
# A login *non-interactive* shell sources zshenv -> zprofile -> profile.d but
# NOT zshrc, so ~/.env (secrets) is deliberately excluded.
set -u
...
"${SHELL:-/bin/zsh}" -lc /usr/bin/env 2>/dev/null | while IFS='=' read -r name value; do
  case "${name}" in
